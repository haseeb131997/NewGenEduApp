...
class StudentProgressCard extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      serviceType: 'Student',
      summaryService: 'StudentProgressCardSummary',
      ...
      errorField: [],
      breadcrumb: 'Menu / Student Level Tracking / Progress Card',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('Student Progress Card api is being canceled');
  ...
  async setMount(unMount) {
    ...
    this.setState({
      ...
      breadcrumb: (globalData.userType == 'P' || globalData.userType == 'S') ? 'Menu / Exam / Progress Card' : 'Menu / Student Level Tracking / Progress Card'
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"You can search a student's progress card by mentioning student's name & exam."}`}
