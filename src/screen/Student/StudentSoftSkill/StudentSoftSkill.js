...
class StudentSoftSkill extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      primaryKeyCols: ['studentID','exam','class'],
      ...
      serviceType: 'Student',
      summaryService: 'StudentSoftSkillSummary',
      ...
      errorField: [],
      breadcrumb: 'Menu / Student Level Tracking / Soft Skills',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('Student Soft Skills api is being canceled');
  ...
  async setMount(unMount) {
    ...
    this.setState({
      ...
      breadcrumb: (globalData.userType == 'P' || globalData.userType == 'S') ? 'Menu / Exam / Soft Skills' : 'Menu / Student Level Tracking / Soft Skills'
    })
  ...
  mandatoryCheck = (operation,currentStep) => {
    if (operation != 'Modification') //N0U-103
     {
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"You can search a student's Soft Skills details by mentioning student's name & exam."}`}
...
export default StudentSoftSkill;
