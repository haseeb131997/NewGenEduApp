...
import EditStudenNotesDetail from '../EditStudenNotesDetail';
...
class StudentNotesDetail extends Component {
  ...
  Mandatory = function () {
    ...
    const { studentNoteEmptyrecord } = stateObject.state

    stateObject.state.errorField = []
    var mandatoryCheckError = false;

    if (studentNoteEmptyrecord.notes == '' || studentNoteEmptyrecord.notes == null) {
  ...
  onSubmit() {
    ...
    console.log('Notes')
    if (this.Mandatory()){
      Paggination.functions.addAndedit(stateObject, 'studentNotes', studentNoteEmptyrecord)
    }
      // notesView = true
  }
  ...
  onClickNew(){
    ...
    var studentNoteEmpty = {
      date: moment(new Date).format('DD-MM-YYYY'),
      notes: "",
      check: false
    }
  
    Paggination.functions.onClickNew(stateObject, 'studentNoteEmptyrecord', studentNoteEmpty)
    // notesView = false
  }
  ...
  onEdit(item, index){
    Paggination.functions.editModaltwo = true
    const { stateObject } = this.props
    Paggination.functions.edit(stateObject, 'studentNoteEmptyrecord', item, index)
    // notesView = false
  }
  ...
  onDelete(index){
   Paggination.functions.editModaltwo = true
    const { stateObject } = this.props
    Paggination.functions.delete(stateObject, 'studentNotes', index)
    // notesView = false
  }
  ...
  render() {
    ...
    return (
      <View style={{}}>
        ...
        {(stateObject.state.currentOperation == 'Modification' && dataModel.studentNotes.length == 0) && <View style={AppStyles.marginTop_2}>
          <CustomButtons
        ...
        {(stateObject.state.currentOperation == 'Modification' && dataModel.studentNotes.length != 0) && <View style={[AppStyles.flex_End, AppStyles.marginTop_1]}>
          <CustomButtons
            onPress={() => this.onClickNew()}
            title={'Add'}
            containerStyle={AppStyles.signInContainer}
        ...
        {(dataModel.studentNotes == 0 && (stateObject.state.currentOperation == 'Modification')) && <ImpNotes
          isArray={false}
          message={`By clicking 'Add Notes' you can add special notes about student`}
        />}

        {dataModel.studentNotes != 0 ?
          ...
          //    />
          <View>
            {dataModel.studentNotes.map((item, index) => (
          ...
          : stateObject.state.currentOperation != 'Modification' ? <View style={AppStyles.alignItems}><Caption>There is no special notes for the student</Caption></View> : null}


        {Paggination.functions.editModaltwo && <SecondModal
          templates={<EditStudenNotesDetail stateObject={stateObject} />}
