...
class StudentNotification extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      primaryKeyCols: ['studentID', 'notificationID'],
      ...
      serviceType: 'Student',
      summaryService: 'StudentNotificationSummary',
      ...
      errorField: [],
      breadcrumb: 'Menu / Student Level Tracking / Notification',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('Student Notification api is being canceled');
  ...
  async setMount(unMount) {
    ...
    this.setState({
      ...
      breadcrumb: (globalData.userType == 'P' || globalData.userType == 'S') ? 'Menu / General / Notifications' : 'Menu / Student Level Tracking / Notifications'
    })
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"You can search a student's Notification details by mentioning student's name"}`}
