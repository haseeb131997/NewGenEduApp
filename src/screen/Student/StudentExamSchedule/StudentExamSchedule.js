...
class StudentExamSchedule extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      serviceType: 'Student',
      summaryService: 'StudentExamScheduleSummary',
      ...
      errorField: [],
      breadcrumb: 'Menu / Student Level Tracking / Exam Schedule',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('Student Exam Schedule api is being canceled');
  ...
  async setMount(unMount) {
    ...
    this.setState({
      ...
      breadcrumb: (globalData.userType == 'P' || globalData.userType == 'S') ? 'Menu / Exam / Exam Schedule' : 'Menu / Student Level Tracking / Exam Schedule'
    })
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"You can search a student's Exam Schedule by mentioning student's name"}`}
