...
class ClassProgressCard extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      errorField: [],
      breadcrumb: 'Menu / Classroom Activities / Parent Signature Tracking/Progress Card',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('class Progress Card api is being canceled');
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"You can track parent's signature on progress card by mentioning the class & exam."}`}
