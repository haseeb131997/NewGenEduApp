...
const Instruction = [
  {
    text:'There is no timetable configured yet.',
    color:UiColor.ERROR_COLOR
  },
  {
    text:' Click "Create Timetable" to configure timetable for a class.',
...
class InstituteYearConfiguration extends React.Component {
  // static onEnter() {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      heading: 'Timetable',
 // periodTimingsEmptyrecord state configuration
      ...
      summaryService: 'ClassTimeTableSummary',
      ...
      createStepsHeading: ['Basic Info','Timetable' ,'Remarks', 'Completed'],
      summaryResultByFilter: [],
      errorField: [],
      breadcrumb:'Menu / Configuration / Timetable',
      viewDetail:null,
      showComplete:false,
 ...
 async setUnmount(unMount) {
   ...
   await apiCall.functions.cancelToken.cancel('Timetable api is being canceled');
  ...
  async setMount(unMount) {
    ...
    emptyDataModel.instituteID = globalData.instituteID
     ...
     this.setState({
      ...
      periodTimingsEmptyrecord:periodTimingsEmptyrecord,
      currentStep: 1,
      currentOperation: 'SummaryQuery',
      intialFetching: true,
  ...
  async beforeApicall(Operation, currentStep, apiObject) {
    ...
    switch (Operation) {
      case 'Create':
        switch (currentStep) {
          case 1:
            // NewOperation.functions.createDefault(this)
            break
        }
        break
      case 'Query':
  ...
  afterApiCall(Operation, currentStep) {
   ...
   getTimeTable() {
    ...
    this.state.timeTableshowResults = []

    var temArray = GeneralUtils.functions.getSummaryResult(this)
    for (let item of temArray) {
      item.selectedDay = 'Mon'
    }
    this.setState({
      timeTableshowResults: temArray
    })
  }
      ...
      <Provider>
        <ScreenTemplate
          stateObject={this}
           ...
           viewHeading ={`${dataModel.classDesc}`}
