...
class ClassAttendanceCalendar extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  getAttendanceDetails(date) {
    ...
    for (let i = 0; i < stateObject.state.childViewDetails.attendanceDetails.length; i++) {
      ...
      if (stateObject.state.childViewDetails.attendanceDetails[i].date == newDate) {
        stateObject.state.childViewDetails.attendanceDetails[i].class = stateObject.state.childViewDetails.classID
        stateObject.state.childViewDetails.attendanceDetails[i].classDescription = stateObject.state.childViewDetails.classDescription
        return stateObject.state.childViewDetails.attendanceDetails[i]
      }
    }
  }

  getEmptyAttendanceDetails(date) {
    const {
      stateObject
    } = this.props
    var oldDate = date.split('-')
    var newDate = `${oldDate[2]}-${oldDate[1]}-${oldDate[0]}`
   var attendanceDetails ={classID:stateObject.state.childViewDetails.classID,
  ...
  loadItems() {
    ...
    setTimeout(() => {
      for (let i = 0; i < stateObject.state.childViewDetails.attendanceDetails.length; i++) {
        var olddate = stateObject.state.childViewDetails.attendanceDetails[i].date.split('-')
        var newDate = `${olddate[2]}-${olddate[1]}-${olddate[0]}`
      ...
      for (var k = 0; k < Difference_In_date; k++) {
        ...
        if (dataModelDate.indexOf(new_startDate) !== -1) {
          this.state.items[new_startDate].push({
            status: true,
            details: this.getAttendanceDetails(new_startDate),
            date: new_startDate,
          })
        }
        else {
          this.state.items[new_startDate].push({
           ...
           // details: stateObject.state.childViewDetails,
            details:this.getEmptyAttendanceDetails(new_startDate),
            date: new_startDate,
          });
        }
      }
  ...
  async addNewAttendance(item) {
    ...
    dataModel.class = item.details.classID
  ...
  renderItem(item) {
    ...
    return item.status ? (
        ...
        <View style={AppStyles.margin_2}>
         ...
         <Image resizeMode='contain' style={[AppStyles.inactiveMoreIcon, AppStyles.succusImgColor]}
          ...
          />
            ...
            <View style={[AppStyles.marginLeft_1,AppStyles.flexDirectionRow,AppStyles.alignItems]}><Text style={AppStyles.textColor}>No. of students present : </Text>
            <Caption style={[AppStyles.successBatchStyle]}>{item.details.presentStudents}</Caption>
            </View>
         </View>
         ...
         <Image resizeMode='contain' style={[AppStyles.inactiveMoreIcon, AppStyles.errorImgColor]}
            source={require('./../../../../asssets/icons/com006.png')}
          />
            <View style={[AppStyles.marginLeft_1,AppStyles.flexDirectionRow,AppStyles.alignItems]}><Text style={AppStyles.textColor}>No. of students absent : </Text>
            <Caption style={[AppStyles.errorBatchStyle]}>{item.details.absentStudents}</Caption>
            </View>
         </View>
      ...
      <View style={styles.mainContainer}>
        <View style={AppStyles.margin_2}>
          <Subheading>There is no attendance</Subheading>
          <View style={[AppStyles.alignItems, AppStyles.marginTop_3]}>
            <CustomButtons
              onPress={() => this.addNewAttendance(item)}
