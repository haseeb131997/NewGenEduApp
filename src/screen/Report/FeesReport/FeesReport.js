...
class FeesReport  extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      primaryKeyCols: ['studentID','class','feeID'],
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('Fees report api is being canceled');
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <FilterScreenTemplate
          ...
          viewHeading={`${"Mention atleast one of the filter to view the report of that corresponding records. If you want fee report for a particular class, then mention the class in the filter. If you want fee report for a particular type of fee, mention the fee description in the filter"}`}
