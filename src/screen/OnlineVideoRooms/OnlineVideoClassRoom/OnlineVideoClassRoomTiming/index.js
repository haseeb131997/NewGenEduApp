...
class OnlineVideoClassRoomTiming extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false
  ...
  render() {
    ...
    return (stateObject.state.currentOperation == 'Create' || stateObject.state.currentOperation == 'Modification') ? (<View style={[AppStyles.marginTop_2]}>
      ...
      <CustomDatePicker
        tooltipReq={true}
        tooltipMsg={'Specify the date on which the online class is to be schedule'} 
      ...
      <CustomTimePicker
        tooltipReq={true}
        tooltipMsg={'Enter the time when the online class is to be started'}//N0U-109
        required={true}
        editable={editable}
        label={'Starting Time'}
...
{/*} // N0U-109 starts
      <NewScreenDropDownPicker
        tooltipReq={true}
        tooltipMsg={'Enter the amount of time the class is supposed to be held.'} //N0U-109
        editable={editable}
        required={true}
        label={'Duration Min'}
      ...
      */
      <CustomTimePicker
        tooltipReq={true}
        tooltipMsg={'Enter the Meeting end time '}//N0U-109
        required={true}
        editable={editable}
        label={'End Time'}
...
export default OnlineVideoClassRoomTiming;
