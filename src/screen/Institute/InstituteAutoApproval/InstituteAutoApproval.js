...
import InstituteGeneralAutoApproval from "./InstituteGeneralAutoApproval";
...
class InstituteAutoApproval extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      heading: 'Auto-Approval Configuration',


      //  ServicesEmptyrecord detail paggination state configuration
      ServicesEmptyrecord: null,
      ...
      serviceName: 'AutoAuthConfiguration',
      serviceType: 'User',
      ...
      createStepsHeading: ['Auto Approval Configuration'],
      summaryResultByFilter: [],
      errorField: [],
      breadcrumb: 'Menu / Configuration / Auto-Approval Configuration',
      viewDetail: null,
      showComplete: false,
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('autoApproval config api is being canceled');
  ...
  async setMount(unMount) {
    ...
    //  ServicesEmptyrecord detail paggination empty record
    var ServicesEmptyrecord = {
      serviceName: "",
      serviceDescription: "",
      status: false
    }
    ...
    dummyDataModel.userType = globalData.userType
    emptyDataModel.userType = globalData.userType
    ...
    this.setState({
      ...
      ServicesEmptyrecord: ServicesEmptyrecord,
      currentStep: 1,
      currentOperation: 'singleRecordView',
      intialFetching: true,
  ...
  async callbackend(operationtype, index) {
    ...
    if (operationtype == 'editServices') {
      if (index != null) {
        apiObject.datamodel.services[index] = this.state.ServicesEmptyrecord
      }
      else {
        apiObject.datamodel.services.push(this.state.ServicesEmptyrecord)
      }

    }
    else if (operationtype == 'deleteServices') {
      apiObject.datamodel.services.splice(index, 1);
    }
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <InstituteScreenTemplate
          ...
          stateObject={this}
          autoApproval={<InstituteGeneralAutoApproval stateObject={this} />}
        />
      </Provider>
    ) : null
  }
