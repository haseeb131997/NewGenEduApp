...
const Instruction = [
  {
    text: 'There are no instant notifications delivered yet.',
    color: UiColor.ERROR_COLOR
  },
  {
    text: 'Click " Create Instant Notifications" to deliver instant notifications for a class/group.',
    color: UiColor.DRAK_GRAY_COLOR
  },
  {
    text: "Instant Notification Messages are the messages which will be delivered to parents on a given delivery date.",
    color: UiColor.DRAK_GRAY_COLOR
  },
  {
...
class InstituteInstantNotification extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      summaryService: 'NotificationSummary',
      ...
      ivas: '',
      uhtuliak: '',
      CreateDefaultDataModel: { notificationID: '' },

      // starts 3.0 UI/UX
      intialFetching: false,
      createStepsHeading: ['Basic Info', 'Message','Remarks', 'Completed'],
      summaryResultByFilter: [],
      errorField: [],
      breadcrumb: 'Menu / Notification / Instant Notification',
      viewDetail: null,
      showComplete: false,
      ...
      welComeTitle: 'Instant Notification',
  ...
  async setUnmount(unMount) {
    ...
    await apiCall.functions.cancelToken.cancel('instant notification api is being canceled');
      ...
      case 'Create-Default':
        switch (currentStep) {
          case 0:
            ...
            dummyModel.notificationID = CreateDefaultDataModel.notificationID
            this.setState({
         ...
         case 1:
          dataModel.templateID = CreateDefaultDataModel.templateID 
          dataModel.bodyTemplate = CreateDefaultDataModel.bodyTemplate,
          dataModel.subjectTemplate = CreateDefaultDataModel.subjectTemplate,
          dataModel.greetingTemplate =CreateDefaultDataModel.greetingTemplate,
          dataModel.footerTemplate = CreateDefaultDataModel.footerTemplate,
          //dataModel.notificationType = CreateDefaultDataModel.messageType;
          //dataModel.messageType = CreateDefaultDataModel.messageType,
          dataModel.messageTypeDescription = CreateDefaultDataModel.messageTypeDescription,
          dataModel.tagDetails = CreateDefaultDataModel.tagDetails,
          //apiCallRequired = !1,
            //fn_Show_Exception_With_Param("FE-VAL-101", [])  
            this.setState({
              dataModel: dataModel
            })

            Exception.functions.showFrontendError(this.parentStateChange, [{ errorCode: 'FE-VAL-111', errorMessage: '', errorParam: '' }])
    ...
    return (unMount == false && dataModel != null) ? (
      <Provider>
        <ScreenTemplate
          ...
          //viewHeading={dataModel.templateDescription}
          viewHeading={`${SelectListUtils.functions.getSelectedValue(SelectListUtils.functions.selectMaster.NotificationMaster, (dataModel.notificationType==''?dataModel.messageType:dataModel.notificationType))}`}
        ...
        <ChildRecordListModal
          ...
          viewHeading={childViewDetails != null && `${SelectListUtils.functions.getSelectedValue(SelectListUtils.functions.selectMaster.NotificationMaster, (childViewDetails.notificationType==''?childViewDetails.messageType:childViewDetails.notificationType))}`}
          CreateTemplate ={CreateTemplate.functions.CreateConfig(this)}
          EditTemplate={EditTemplate.functions.ModificationConfig(this)}
          ViewTemplate={ViewTemplate.functions.QueryConfig(this)}
          AuthTemplate={AuthTemplate.functions.AuthorisationConfig(this)}
          queryTitle={dataModel.templateDescription}
          querySubTitle={`${dataModel.instant}`}
        />
