...
export default class FilterCustomDatePicker extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showDate: false,
  ...
  onDateChange(event, selectedDate){
    ...
    var selectDate = currentDate != '' ? moment(currentDate).format('DD-MM-YYYY') : currentDate
  ...
  render() {
    const { label, secureTextEntry, onChangeText, value, editable, placeholder,onDateChange,mode,format,required,errorMessage,tooltipReq,tooltipMsg,tooltipStyle } = this.props;
    return (
   ...
   <View style={AppStyles.flexDirectionRow}>
      ...
      style={[styles.inputView, AppStyles.marginTop_1]}>
        <View style={styles.iconContainer}>
          <Foundation
            name={"calendar"} size={AppStyles.iconSize.height} color={UiColor.LIGHT_TEXT_COLOR} />
        </View>
        <TextInput
          autoCapitalize='none'
          value={value}
