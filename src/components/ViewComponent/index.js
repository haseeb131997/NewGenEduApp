...
class ViewComponent extends Component {
  ...
  render() {
    const {
      stateObject, templates, stepsHeading, title, viewHeading
    } = this.props
    const { currentOperation } = stateObject.state
    return (
      <Portal>
        <Modal visible={currentOperation == 'View' ? true : false} onDismiss={() => SubScreenUtils.functions.closeModal(stateObject)}
